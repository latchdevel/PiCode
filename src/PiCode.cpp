...
String PiCode::pulseTrainToStringClass(const uint16_t* codes, unsigned int length, uint8_t repeats) {

  bool match = false;
  int diff = 0;

  uint8_t nrpulses = 0;  // number of pulse types
  uint16_t plstypes[MAX_PULSE_TYPES] = {0};

  String data("");
  data.reserve(6 + (unsigned int)length);
  data += "c:";
  for (unsigned int i = 0; i < length; i++) {
    ...
    if (!match) {
      ...
      if (nrpulses > MAX_PULSE_TYPES) {
        return String("ERROR: too many pulse types");
      }
    }
  }
  data += ";p:";
...
char* PiCode::pulseTrainToString(const uint16_t* pulses, unsigned int length, uint8_t repeats){
  
  bool match = false;
  int  diff  = 0;

  uint8_t  nrpulses                  =  0 ;  // number of pulse types
  uint16_t plstypes[MAX_PULSE_TYPES] = {0};  // array to store pulse types
...
/* String class */ /* Encode protocol and json parameters to array of pulses if success */
int PiCode::encodeToPulseTrain(uint16_t* pulses, protocol_t* protocol, const String& json_data) {
  
  int result = PiCode::ERROR_UNAVAILABLE_PROTOCOL;

  /* local copy of json_data */
  String n_json = json_data;

  /* replace simple ' to double " */
  n_json.replace('\'', '\"');

  if (!json_validate(n_json.c_str())) {
    ...
    if ( (protocol != nullptr) && (protocol->createCode != nullptr) ) {  // always (protocol->maxrawlen <= std::numeric_limits<typeof(protocol->maxrawlen)>::max()
      ...
      JsonNode *message = json_decode(n_json.c_str());
...
int PiCode::encodeToPulseTrain(uint16_t* pulses, protocol_t* protocol, const char* json_data){
...
int PiCode::stringToPulseTrain(const String& data, uint16_t* codes, size_t maxlength) {
...
int PiCode::stringToPulseTrain(const char* data, uint16_t* pulses, size_t maxlength){
  ...
  while (end > 0) {
    ...
    if ((unsigned)end-start > sizeof(pulse_str)-1) {
      ...
      plstypes[nrpulses++] =  (uint16_t)atoi(pulse_str);
    }
    start = (unsigned)end + 1;
...
String PiCode::decodePulseTrainToString(uint16_t* pulses, uint8_t length, const char* indent) {
...
/* Decode from array of pulses to String */
char* PiCode::decodePulseTrain(uint16_t* pulses, uint8_t length, const char* indent){
size_t matches = 0;
...
String PiCode::decodeString(const String& pilight_string){

  char* p_result = decodeString(pilight_string.c_str());

  String result = String(p_result);

  if (p_result) free(p_result);
...
String PiCode::encodeJson(const String& json, uint8_t repeats ){

  char* p_result = encodeJson(json.c_str(), repeats);
